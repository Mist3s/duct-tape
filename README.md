...
Набор утилит для повседневной работы медстатистика: реестры ОМС (DBF), статистика и
экономика стационара, смертность, отчёты в Excel и HTML. Название говорит о подходе:
синяя изолента — как швейцарский нож, одна программа на все мелкие задачи, которые
иначе делаются руками.

Проект — платформа: новые утилиты добавляются одним файлом-плагином и автоматически
получают вкладку, сохранение настроек и общий журнал работы. Программа сама проверяет
выход новой версии на GitHub и умеет обновляться в один клик.
...
Последний пункт бокового списка — **«О программе»**: название, версия, автор, лицензия,
ссылка на GitHub и кнопки «Проверить обновление», «Обновить», «Страница релиза».

- **При запуске** программа сама проверяет, вышла ли новая версия (галочка
  «Проверять обновления при запуске» на той же странице, ключ
  `app.check_updates` в `настройки.txt`). Проверка идёт в фоновом потоке
  с коротким таймаутом. Если сети нет или GitHub закрыт, **никаких окон с ошибками не
  появляется** — статус просто виден на странице «О программе». Когда обновление
  найдено, пункт списка помечается номером версии, а в журнал добавляется строка.
- **Кнопка «Обновить»** скачивает новый `duct-tape.exe`, сверяет его **sha256 с суммой,
  опубликованной в API релиза**, заменяет им себя и перезапускается. Прежняя версия
  сохраняется рядом как `duct-tape.old.exe` и удаляется при следующем запуске.
  Если контрольная сумма не совпала — файл удаляется и установка отменяется; если не
  удалось подменить файл (например, нет прав в `Program Files`) — прежняя версия
  возвращается на место, а на странице появляется объяснение.
- При запуске **из исходников** обновление не выполняется: программа предлагает
  `git pull`.

Механика — в [`core/updater.py`](src/omsreg/core/updater.py) (только стандартная
библиотека), интерфейс — в [`gui/about.py`](src/omsreg/gui/about.py).
...
src/omsreg/
  ...
  gui/      платформа: spec, registry, theme, config, log_panel, app, plugin_fields
              (общие поля вкладок), about (страница «О программе» и обновление),
              plugins/ — только модули-плагины: один файл = одна вкладка
...
Набор правил ruff (`E F W I UP B N D FA ARG SIM RET C4 PERF PTH PLC0415 RUF100 C90
...
**Минимальная версия — Python 3.9**, и CI гоняет тесты на 3.9–3.13. Часть ошибок видна
только на старых версиях (например, `str | None` в аннотации вычисляется при импорте, а
`urllib.error.HTTPError` без тела ответа на 3.9 открывает временный файл). Проверить
самую старую версию локально, ничего не устанавливая в систему:

```bash
uv run --python 3.9 --with pytest --no-project pytest -q
```

Правило `FA` в ruff ловит первый из этих случаев ещё до запуска тестов.
